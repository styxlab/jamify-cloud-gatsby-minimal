...
const { createMediaRef } = require(`./create-media-ref`)
...
exports.onCreateNode = async function ({
    ...
    const { lookup, target, exclude, failOnMissing, verbose } = _.merge({}, pluginDefaults, pluginOptions)
    ...
    const lookupNodes = lookup.filter(item => item.nodeType === node.internal.type)
    ...
    const allImgTags = lookupNodes[0].imageTags.filter(item => node[item] !== null && node[item] !== undefined)
    ...
    try {
        ...
        reporter.panicOnBuild(`Error processing images ${node.url ?
            `image ref ${node.url}` : `in node ${node.id}` }:\n ${err}`)
        return {}
    }
