...
async function processRemoteNode({
  ...
  const {
    ...
    reponseHeaders,
    buffer
  } = await requestRemote(url, headers, httpOpts);
  let digest;
  ...
  const mediaNode = await createMediaRefNode(url, createNodeId, {}); // extension

  mediaNode.ext = ext || mediaNode.ext;
  mediaNode.extension = ext.slice(1).toLowerCase();
  mediaNode.mediaType = mime.getType(ext); // internal
  ...
  mediaNode.parent = parentNodeId; // Add image/media meta
  ...
  await createNode(mediaNode, {
    name: `gatsby-transformer-media-ref`
  });
  return mediaNode;
}
/**
...
const createMediaRef = ({
  ...
  if (!url || isWebUri(url) === undefined) {
    return Promise.reject(`url passed to CreateMediaRef is either missing or not a proper web uri: ${url}`);
  }
